...
public class Decoder {
  ...
  /* Modified by RH Dec 16th 2014 begins */
  //public static final int DEFAULT_PARALLELISM = 4;
  public static final int DEFAULT_PARALLELISM = 1;
  /* Modified by RH Dec 16th 2014 ends */
  ...
  long fixErasedBlockImpl(FileSystem srcFs, Path srcFile, FileSystem parityFs,
    ...
    /* Added by RH Jan 20th 2015 begins */
    //LOG.info("rhDebugMsg: startTime: " + System.currentTimeMillis());
    /* Added by RH Jan 20th 2015 ends */
    long decodingTime = 0;
    if (crc != null) {
      crc.reset();
    }
    ...
    try {
      ...
      /* Added by RH Jan 20th 2015 begins */
      LOG.info("rhDebugMsg: endTime: " + System.currentTimeMillis());
      /* Added by RH Jan 20th 2015 ends */
  ...
  class DecodeOp implements Runnable {
    ...
    public void run() {
      try {
        //LOG.info("RHDEBUGMSG: before Decode" + System.currentTimeMillis());
        //LOG.info("RHDEBUGMSG: startIdx" + startIdx + ", count" + count);
        code.decodeBulk(readBufs, writeBufs, erasedLocations, startIdx, count);
        //LOG.info("RHDEBUGMSG: after Decode" + System.currentTimeMillis());
      } catch (IOException e) {
  ...
  public class DecoderInputStream extends InputStream {
    ...
    private void init() throws IOException {
      if (startTime == -1) {
        ...
        //LOG.info("RHDEBUGMSG: startTime " + startTime);
      }
      ...
      if (null == parallelReader) {
         ...
         * We do not read from some locations does not mean we lost them!!! */
        //for (int location : locationsToNotRead) {
        for (int location : erasedLocations) {
        /* fixd by RH Dec 15th 2014 ends */
          locationsToFix[i] = location;
          i++;
        }
        /* added by RH for debug Dec 15th 2014 begins */
        //LOG.info("erased count: " + i + System.currentTimeMillis());
        for (int j=i;j<codec.parityLength;j++) {
          locationsToFix[j]=-1;
        }
        /* added by RH for debug Dec 15th 2014 ends */
      ...
      if (null == buffer) {
        ...
        doParallelDecoding(toWrite, readResult, parallelCode, locationsToFix);
        long decTime = System.currentTimeMillis() - startDecoding;
        //LOG.info("RHDEBUGMSG: decoding time for packet: " + decTime);
        ...
        decodingTime += (System.currentTimeMillis() - startDecoding);
        /* Added by RH Jan 20th 2015 begins */
        double fromStart = System.currentTimeMillis() - startTime;
        //LOG.info("RHDEBUGMSG: from Start " + fromStart);
        /* Added by RH Jan 20th 2015 ends */
      }
    }
